...
wrappers or foreground/frame-pumping tricks are installed.
...
const ticket = await agent.start(scene.viewRevision, {
...
// Continue consuming; inspect event.result for the canonical input outcome.
await agent.close();
```
...
2. An `action` event carries the normal canonical dispatch, observation, runtime
   ...
   execution time. Failures halt further input. Reconcile unknown dispatch;
   never blindly retry with another identity.
...
scroll-to, back, and dismiss, with normal typed parameters. Agent mode rejects
expectation parameters, safety overrides, file input, capture and screenshots.
Pass sensitive input through JSON stdin, never process arguments. Ordinary
finite Scout commands still support atomic same-call expectations when useful.
...
## Raw JSONL contract (agentProtocol 1)
...
next (`timeoutMs` 0–30,000), cancel, close. Raw request lines are at most 64 KiB;
